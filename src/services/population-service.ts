...
export class PopulationServiceImpl implements PopulationService {
    ...
    async getJsonContent(response: Response): Promise<unknown> {
        ...
        try {
            ...
            throw new Error(`Could not parse the response body as JSON. Error: ${(error as any).message}`);
        }
        return jsonContent;
    }
    ...
    async getAllCountries(): Promise<Country[]> {
        const response: Response = await fetch(`${this.countriesApiBaseUrl}?${WorldBankApiV2Params.FORMAT}=${WorldBankApiV2Formats.JSON}&${WorldBankApiV2Params.PER_PAGE}=320`);
